...
### Подсказка к заданию 9

Число _N_ можно считывать в переменную типа _int_.
В этой задаче вам может пригодиться контейнер _vector_.
Напоминаем, что добавление значения _x_ в конец вектора _v_ делается так:
